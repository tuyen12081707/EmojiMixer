...
class ResultCollectionActivity : BaseActivity<ActivityResultCollectionBinding>(ActivityResultCollectionBinding::inflate) {
    ...
    private fun mixEmojis(emoji1: String, emoji2: String, date: String) {
        ...
        val em = EmojiMixer(emoji1, emoji2, date, this, object : EmojiMixer.EmojiListener {
            override fun onSuccess(emojiUrl: String) {
                ...
                var listCollection = Common.getListCollection(this@ResultCollectionActivity)
                listCollection.add(emojiUrl)
                Common.setListCollection(this@ResultCollectionActivity,listCollection)
    ...
    private fun setImageFromInt(image: ImageView, url: Int) {
        Glide.with(this)
            .load(url)
            .fitCenter()
            .transition(DrawableTransitionOptions.withCrossFade())
    ...
    private fun shouldShowEmoji(shouldShow: Boolean) {
        if (shouldShow) {
            UIMethods.shadAnim(binding.mixedEmoji, "scaleY", 1.0, 300)
            UIMethods.shadAnim(binding.mixedEmoji, "scaleX", 1.0, 300)
            UIMethods.shadAnim(binding.progressBar, "scaleY", 0.0, 300)
            UIMethods.shadAnim(binding.progressBar, "scaleX", 0.0, 300)
        } else {
            UIMethods.shadAnim(binding.mixedEmoji, "scaleY", 0.0, 300)
            UIMethods.shadAnim(binding.mixedEmoji, "scaleX", 0.0, 300)
            UIMethods.shadAnim(binding.progressBar, "scaleY", 1.0, 300)
            UIMethods.shadAnim(binding.progressBar, "scaleX", 1.0, 300)
        }
    }
